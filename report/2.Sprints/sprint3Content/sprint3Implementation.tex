...
Since the new servers utilizes the Docker Swarm, the first order in solving the issue is to create a new service in the Swarm. This is done within the \text{docker-compose.yml} file. 
...
Once the service had been started the only thing left was to create an dump from the existing MySQL server. 
